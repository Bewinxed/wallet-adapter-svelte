<script lang="ts">
	...
	const wallet = $derived(context.wallets.find((wallet) => wallet.adapter === context.selectedAdapter) ?? null);
	...
	$effect(() => {
		...
		if (
			...
			context.wallet.status === 'connecting' || context.wallet.status === 'connected' ||
			!onAutoConnectRequest ||
			!(
