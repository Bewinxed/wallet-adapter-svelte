<script lang="ts">
	...
	const AppIdentityUri = globalThis.location
	...
	const adaptersWithMobileWalletAdapter = $derived(
		mobileWalletAdapter == null
			...
			: adaptersWithStandardAdapters.concat(mobileWalletAdapter)
	);

	const localStorage = useLocalStorage(
		localStorageKey,
		isMobile ? SolanaMobileWalletAdapterWalletName : null
	);

	let walletName = $state(localStorage.value);

	const adapter = $derived(
		(mobileWalletAdapter == null || adaptersWithStandardAdapters.indexOf(mobileWalletAdapter) !== -1
			? adaptersWithStandardAdapters
			: adaptersWithStandardAdapters.concat(mobileWalletAdapter)
		).find((a) => a.name === walletName) ?? null
	);
